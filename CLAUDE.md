...
**記錄使用者提問與發現（重要工作規則）**：當使用者在對話中提出關於 Cowork 機制的問題、且過程中驗證出結果或發現（尤其是推翻/更正先前記錄的），**一律補進對應子檔**，不要只在對話裡回答完就算。記錄時：
- 標明「問題起點」（使用者問了什麼），讓後續讀者知道這條發現的脈絡
- 標明來源與日期（即時內省／JSONL／binary…，見下方驗證方法）
- 若是更正先前的過度宣稱或錯誤，**明確標「更正」並保留「原本錯在哪、為何更正」**，不要默默改掉
- 同步更新 `analysis.md` 索引的對應條目
